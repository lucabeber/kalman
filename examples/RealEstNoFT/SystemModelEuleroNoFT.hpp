...
namespace KalmanExperimentsNoFT
{
...
public:
	...
	typedef KalmanExperimentsNoFT::Estimation::State<T> S;
    
    //! Control type shortcut definition
    typedef KalmanExperimentsNoFT::Estimation::Control<T> C;
    ...
    S f(const S& x, const C& u) const
    {
        ...
        x_.x1() = x.x1() + x.x2()*dT;
        x_.x2() = x.x2() - dT/(m)*( k0*u.u() + x.x1()*(x.x3()) + x.x2()*(c0+x.x4()) );
        x_.x3() = x.x3();
        x_.x4() = x.x4();
