...
public:
    ...
    //! Global posision of the ee in z direction
    ...
    static constexpr size_t SURFACE = 4;
...
public:
    ...
    S f(const S& x, const C& u) const
    {
        ...
        x_.x1() = x.x1() + x.x2()*dT;
        x_.x2() = x.x2() + dT/(m)*( k0 * ( u.u() - x.x1()) + (x.x5() - x.x1())*(x.x3()) - x.x2()*(x.x4()+c0) );
        x_.x3() = x.x3();
        x_.x4() = x.x4();
        x_.x5() = x.x5();
    ...
    {   
        ...
        this->F( S::VELOCITY, S::POSITION ) = -(dT * (-k0 -x.x3())) / (m);
        // partial derivative of x.x2() w.r.t. x.x2()
        this->F( S::VELOCITY, S::VELOCITY ) = 1 - (dT * (x.x4()+c0)) / (m);
        // partial derivative of x.x2() w.r.t. x.x3()
        this->F( S::VELOCITY, S::ELASTICITY ) = (dT * (-x.x1() + x.x5())) / (m);
        // partial derivative of x.x2() w.r.t. x.x4()
        this->F( S::VELOCITY, S::VISCOSITY ) = -(dT * x.x2() )/ (m);
        // partial derivative of x.x2() w.r.t. x.x5()
        this->F( S::VELOCITY, S::SURFACE ) = (dT * (x.x3())) / (m);
